...
namespace NMJToolBox
{
    public static class NaughtyEditorGUI
    {
        ...
        public static void DrawButtonsForInspector(Object target, IEnumerable<MethodInfo> methodInfos)
        {
            var methods = methodInfos.ToArray();
            //0 = Horizontal , 1 = Vertical
            var controlGroup = new List<int>();

            foreach (MethodInfo method in methods)
            {
                Attribute[] reverse = method.GetCustomAttributes(typeof(ButtonBeginHorizontal)).ToArray();
                ...
                {
                    int index = reverse.Length is 0 ? attributes.Length - i - 1 : i;
                    ...
                    else if (attributeType == typeof(ButtonBeginHorizontal))
                    {
                        ButtonBeginHorizontal beginHorizontal =
                            (ButtonBeginHorizontal) attribute;
                        ...
                        {
                            GUILayout.BeginHorizontal(beginHorizontal.text, GUI.skin.window,
                                                      GUILayout.MaxHeight(1));
                        }

                        controlGroup.Add(0);
                    }
                    else if (attributeType == typeof(ButtonEndHorizontal))
                    {
                    ...
                    else if (attributeType == typeof(ButtonBeginVertical))
                    {
                        ButtonBeginVertical beginVertical = (ButtonBeginVertical) attribute;
                    ...
                    else if (attributeType == typeof(ButtonEndVertical))
                    {
                        int lastIndex = controlGroup.Count - 1;
            ...
            for (var i = controlGroup.Count - 1; i >= 0; i--)
            {
                if (controlGroup[i] == 0)
                {
                    GUILayout.EndHorizontal();
        ...
        private static void DrawButton(Object target, MethodInfo methodInfo)
        {
        ...
        public static void NativeProperty_Layout(UnityEngine.Object target, PropertyInfo property)
        {
            ...
            if (value == null)
            {
                string warning = string.Format("{0} is null. {1} doesn't support reference types with null value",
        ...
        public static void HelpBox_Layout(string message, MessageType type, UnityEngine.Object context = null,
            bool logToConsole = false)
        {
