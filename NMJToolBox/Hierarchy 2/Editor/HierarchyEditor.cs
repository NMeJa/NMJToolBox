...
namespace Hierarchy2
{
    ...
    public sealed class HierarchyEditor
    {
        ...
        void ElementAsSeparator(bool isChild = false)
        {
            if (currentEvent.type != EventType.Repaint)
                return;
            ...
            Color guiColor = GUI.color;
            GUI.color = isChild ? ThemeData.colorHeaderBackgroundInChildren : ThemeData.colorHeaderBackground;
                    ...
                    menuLayers.AddItem(new GUIContent(layer),
                                       LayerMask.NameToLayer(layer) == gameObject.layer ? true : false, () =>
                                       {
                                           if (settings.applyLayerTargetAndChild)
                                               ApplyLayerTargetAndChild(
                                                   gameObject.transform, LayerMask.NameToLayer(layer));
                                           else
                                           {
        ...
        internal sealed class MenuCommand
        {
            ...
            static void CreateHeaderInstance(UnityEditor.MenuCommand command)
            {
                GameObject gameObject =
                    new GameObject(string.Format("{0}Separator", HierarchyEditor.instance.settings.separatorStartWith));
